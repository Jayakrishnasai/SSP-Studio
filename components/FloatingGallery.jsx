...
const GalleryImage = memo(function GalleryImage({ src, mouse, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.05, zIndex: 20, transition: { duration: 0.4 } }}
    >
      <motion.div
        animate={{
          x: mouseX + baseX * 0.1,
          y: mouseY + baseY * 0.05,
          rotate: (index % 3 - 1) * 2 + mouseX * 0.5,
      ...
      >
        <img
          ...
          style={{ filter: `brightness(${0.75 + (index % 3) * 0.1}) contrast(1.1)` }}
        />
...
const FloatingGallery = memo(function FloatingGallery() {
  ...
  return (
    <section ref={ref} className="relative py-32 lg:py-44 overflow-hidden bg-ssp-darker">
      ...
      <motion.div style={{ scale, willChange: 'transform' }} className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        <motion.div
        ...
        >
          <p className="text-ssp-accent tracking-[0.3em] uppercase text-sm mb-4">The Gallery</p>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold">
            Frames in <span className="text-gradient">Motion</span>
          </h2>
        </motion.div>

        <div className="relative h-[60vh] md:h-[70vh] w-full">
          {frames.map((src, i) => (
            <GalleryImage key={src} src={src} mouse={mouse} index={i} />
          ))}
        </div>
